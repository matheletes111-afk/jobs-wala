...
import { Menu, X, User, ChevronDown, LogOut, ShieldAlert } from "lucide-react";
...
export default function AdminHeaderNav({
  ...
  const realName = session?.user?.name || userName;
  ...
  return (
    <>
      {/* Desktop nav: capsule dropdown for profile/logout */}
      <div className="hidden items-center gap-3 xl:gap-4 md:flex relative" ref={dropdownRef}>
        ...
        >
          <User className="h-4 w-4 text-slate-550 shrink-0" />
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-11 z-50 w-56 rounded-xl border border-slate-200 bg-white p-2.5 shadow-lg animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <button
              ...
              className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-semibold text-red-650 hover:bg-red-50 hover:text-red-755 transition-colors"
            >
      ...
      <div className="flex items-center gap-3 md:hidden">
        <span className="rounded-full bg-blue-600/10 border border-blue-600/20 px-3 py-1 text-xs font-semibold text-blue-600">
          Admin
        </span>
        <Button
          type="button"
        ...
        >
          <div className="flex-1 overflow-y-auto px-6 py-8">
            ...
            <div className="mt-8 border-t border-slate-100 pt-8 flex flex-col gap-3">
              ...
              <button
                onClick={handleSignOut}
                className="flex w-full h-11 items-center justify-center gap-2 rounded-xl bg-red-50 border border-red-200/60 text-xs font-semibold text-red-650 hover:bg-red-100 transition-colors"
              >
