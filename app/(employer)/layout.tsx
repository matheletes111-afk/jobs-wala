    ...
    <div className="flex min-h-screen w-full min-w-0 flex-col bg-transparent text-foreground selection:bg-primary/30">
      <header className="sticky top-0 z-50 w-full glass border-b border-white/5 shadow-2xl">
        <div className="mx-auto flex h-20 w-full max-w-7xl min-w-0 items-center justify-between gap-4 px-4 sm:px-6 md:px-8 lg:px-10">
          <Link href={isApproved ? "/employer/dashboard" : "/employer/profile"} className="flex shrink-0 items-center gap-3 transition-opacity hover:opacity-80">
      ...
      <main className="min-w-0 flex-1 relative">
        ...
        {!isApproved && (
          <div className="mx-auto w-full max-w-7xl px-4 pt-6 sm:px-6 md:px-8 lg:px-10">
            <div className={`rounded-[1.5rem] border p-6 flex flex-col md:flex-row items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-700 ${
              ...
              <div className="flex items-center gap-4">
                <div className={`h-12 w-12 shrink-0 rounded-2xl flex items-center justify-center border ${
                  ...
                  <svg className={`h-6 w-6 animate-pulse ${
                    dbUser?.employerProfile?.approvalStatus === "REJECTED" ? "text-red-500" : "text-amber-500"
